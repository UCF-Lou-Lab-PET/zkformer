...
r1cs_gg_ppzksnark_keypair<ppT> r1cs_gg_ppzksnark_generator(const r1cs_gg_ppzksnark_constraint_system<ppT> &r1cs)
{
    ...
    // const libff::Fr<ppT> t = libff::Fr<ppT>::one();
    ...
    // std::cout<<non_zero_At<<" "<<non_zero_Bt;
                                                      ...
                                                      const r1cs_gg_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
    ...
    /*check the proving functionality by changing the witness
     * expected:
        prover's incorrect proof (from incorrect witness) can't pass verification
    */
   
    // libff::Fr_vector<ppT> fake_auxiliary_input(0);

    // fake_auxiliary_input.insert(fake_auxiliary_input.end(),auxiliary_input.begin(),auxiliary_input.end());
    // // fake_auxiliary_input.erase(fake_auxiliary_input.begin() + 1);
    // // fake_auxiliary_input.insert(fake_auxiliary_input.begin() + 1, libff::Fr<ppT>::random_element());
    // std::swap(fake_auxiliary_input[1], fake_auxiliary_input[2]);

    // // check assignments
    // size_t assignment_len =auxiliary_input.end()- auxiliary_input.begin();
    // printf("---------witness value (aux input) ---------\n");
    // for(size_t i=0; i < assignment_len; i++){
    //     std::cout <<i+1 <<" ";
    //     auxiliary_input[i].print();
    // }
    // std::cout<<std::endl;

    // size_t assignment_len_1 =fake_auxiliary_input.end()- fake_auxiliary_input.begin();
    // printf("---------witness value (fake uax input) ---------\n");
    // for(size_t i=0; i < assignment_len_1; i++){
    //     std::cout <<i+1 <<" ";
    //     fake_auxiliary_input[i].print();
    // }
    // std::cout<<std::endl;
    ...
    // const qap_witness<libff::Fr<ppT> > qap_wit = r1cs_to_qap_witness_map(pk.constraint_system, primary_input, fake_auxiliary_input, libff::Fr<ppT>::zero(), libff::Fr<ppT>::zero(), libff::Fr<ppT>::zero());
    ...
    // test with a "wrong proof", proof from wrong witness (assignment)
    ...
    // const_padded_assignment.push_back(libff::Fr<ppT>::random_element());
    // const_padded_assignment.insert(const_padded_assignment.end(), qap_wit.coefficients_for_ABCs.begin()+1, qap_wit.coefficients_for_ABCs.end());
