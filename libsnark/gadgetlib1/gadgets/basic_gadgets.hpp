...
private:
    // alpha as an array is everybit of alpha_packed, 0~n-1
    // now we predefiend a table of size 2*n where all possible alpha can be constructed from the table
    pb_variable_array<FieldT> alpha;
    pb_variable<FieldT> alpha_packed;
    std::shared_ptr<packing_gadget<FieldT> > pack_alpha;

    //all_zeros_test: whether the n variables of alpha are all zero
...
template<typename FieldT>
class comparison_gadget_opt : public gadget<FieldT> {
private:
    // alpha as an array is everybit of alpha_packed, 0~n-1
    // now we predefiend a table of size 2*n where all possible alpha can be constructed from the table
    pb_variable_array<FieldT> alpha;
...
public:
    ...
    const pb_variable<FieldT> less;
    const pb_variable<FieldT> less_or_eq;
    // a new construction function
    comparison_gadget_opt(protoboard<FieldT>& pb,
                      const size_t n,
                      const pb_linear_combination<FieldT> &A,
                      const pb_linear_combination<FieldT> &B,
                      const pb_variable<FieldT> &less,
                      const pb_variable<FieldT> &less_or_eq,
                      const pb_variable_array<FieldT> alpha_table,
                      const std::string &annotation_prefix="") :
        gadget<FieldT>(pb, annotation_prefix), n(n), A(A), B(B), less(less), less_or_eq(less_or_eq), alpha_table(alpha_table)
    {
        /* 
        * now, instead of allocating a new array of size n for eveyr alpha
          use a look up table to define alpha
        */
        // alpha.allocate(pb, n, FMT(this->annotation_prefix, " alpha"));
        for(size_t i=0; i < n; i++){
            alpha.push_back(alpha_table[2*i + i%2]);
        }
        alpha.emplace_back(less_or_eq); // alpha[n] is less_or_eq
