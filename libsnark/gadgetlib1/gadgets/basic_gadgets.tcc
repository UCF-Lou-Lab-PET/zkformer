...
void comparison_gadget<FieldT>::generate_r1cs_constraints()
{
    ...
    // if true, pack_alpha is also going to check whether every bit[i] is indeed bolean or not
    // check alpha_i can be packed as original alpha; 
    // and individual alpha_i is binary when this is ensured, this check can be redundant
...
void comparison_gadget<FieldT>::generate_r1cs_witness()
{
    ...
    // std::cout<<(FieldT(2)^n)<<" "<<this->pb.lc_val(B).as_ulong()<<" "<<this->pb.lc_val(A).as_ulong()<<std::endl;
...
// impl for opt
template<typename FieldT>
void comparison_gadget_opt<FieldT>::generate_r1cs_constraints()
{
    /*
      packed(alpha) = 2^n + B - A
    ...
    /* constraints for packed(alpha) = 2^n + B - A */
    // if true, pack_alpha is also going to check whether every bit[i] is indeed bolean or not
    // when this is ensured, this check can be redundant 
    pack_alpha->generate_r1cs_constraints(true);
    this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>(1, (FieldT(2)^n) + B - A, alpha_packed), FMT(this->annotation_prefix, " main_constraint"));
...
template<typename FieldT>
void comparison_gadget_opt<FieldT>::generate_r1cs_witness()
{
    A.evaluate(this->pb);
    B.evaluate(this->pb);
    ...
    // std::cout<<(FieldT(2)^n)<<" "<<this->pb.lc_val(B).as_ulong()<<" "<<this->pb.lc_val(A).as_ulong()<<std::endl;
    this->pb.val(alpha_packed) = (FieldT(2)^n) + this->pb.lc_val(B) - this->pb.lc_val(A);
    // instead of storing every alpha_i, choose from a table
    // pack_alpha->generate_r1cs_witness_from_packed();
    const libff::bigint<FieldT::num_limbs> rint = this->pb.val(alpha_packed).as_bigint();
    // here, this means bits which is array of alpha_i in packing gadget
