...
void protoboard<FieldT>::dump_variables() const
{
    ...
    for (size_t i = 0; i < constraint_system.num_variables(); i++)
    {
        ...
        std::string anno=constraint_system.variable_annotations.find(i+1)->second;
        std::cout<<anno<<" ";
        // std::cout<<constraint_system.variable_annotations.find(i)->first<<" ";
        // value[0] == assignment[0], not one
        if(anno.find("all_zeros_test inv") != std::string::npos){
            std::cout<<values[i].inverse().as_ulong()<<std::endl;
        }else{
            std::cout<<values[i].as_ulong()<<std::endl;
        }
    }
#endif
}
