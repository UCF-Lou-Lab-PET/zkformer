...
linear_combination<FieldT> pb_packing_sum(const pb_linear_combination_array<FieldT> &v)
{
    ...
    //all term is a linear combination
    //v is a array of linear combination
    // std::cout<<"in pk sum v size: "<<v.size()<<std::endl;
    for (auto &lc : v)
    {
        // std::cout<<"in v, lc size: "<<lc.terms.size()<<" ";
        for (auto &term : lc.terms)
        {
            // std::cout<<"in pk sum: "<<twoi.as_ulong()<<std::endl;
